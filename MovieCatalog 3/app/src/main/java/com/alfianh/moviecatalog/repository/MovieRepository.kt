...
class MovieRepository constructor(context: Context) {
  ...
  fun getAllFavoriteMovieForWidget(): List<Movie> {
    val movies = mutableListOf<Movie>()
    favoriteMovieDao.getAllFavoriteMoviesForWidget().forEach {
      movies.add(constructFavoriteToMovie(it))
    }
    return movies
  }
